...
export default function Achive() {
  return (
    <>
      <div className='h-full w-full text-white gap-y-10 flex flex-col justify-center'>
        <p className='text-white text-3xl'>All projects.</p>
              ...
              <tr key={project.githubUrl} className="py-10 hover:bg-[#1e293b]">
                ...
                <td className="flex flex-wrap items-center p-[1rem] text-left py-5 font-gray-500 text-xs whitespace-normal break-words">
                  {project.technologies.map((technology, i) => (
                    <span key={i}>{technology}{''}{i !== technology.length - 1 && <span className="font-extrabold mx-[5px]">&middot;</span>}</span>
                  ))}
                </td>
